      ...
      if (dist <= 3) {
        ...
        for (const bh of blast) {
          ...
          if (dc) {
            dc.hp = (dc.hp || 20) - 15;
            if (dc.hp <= 0) {
              enemyPlayer.cities = enemyPlayer.cities.filter(c => c.id !== dc.id);
              dc.hp = 5; dc.hpMax = 20;
              aiPlayer.cities.push(dc);
              bh.ownerPlayerId = aiPlayer.id;
              addLogFn(`☢ ${dc.name} obliterated by AI!`, g);
            } else {
    ...
    if (city.currentProduction) {
      ...
      if (def && city.productionProgress >= def.cost) {
        if (isUnit) {
          ...
          if (city.currentProduction.itemId === "nuke") aiPlayer.gold -= 10;
        } else {
...
export default function HexStrategyGame(){
  ...
  if(gs.victoryStatus){const w=players.find(p=>p.id===gs.victoryStatus.winner);if(!victoryPlayed.current){victoryPlayed.current=true;SFX.victory();}
    return(<div style={{width:"100vw",height:"100vh",background:"radial-gradient(ellipse at center,#1a2a10 0%,#0a0e06 70%)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",fontFamily:"'Palatino Linotype',serif"}}>
      ...
      <button onClick={()=>{uidCtr=0;setGs(null);setGameStarted(false);setGameMode(null);setSelU(null);setSelH(null);setAiThinking(false);setTutorialOn(true);setTutorialDismissed({});victoryPlayed.current=false;}} style={{...btnStyle(true),marginTop:24,fontSize:14,padding:"8px 24px"}}>New Game</button>
    </div>);}
