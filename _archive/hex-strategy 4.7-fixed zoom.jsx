...
export default function HexStrategyGame(){
  ...
  const onWh=useCallback(e=>{e.preventDefault();e.stopPropagation();
    ...
    // screenPos = pan + screenCenter - (wW*z)/2 + worldX*z  =>  worldX = (mx - pan.x - cx + wW*z/2) / z
    ...
    // mx = newPan.x + cx - (wW*newZ)/2 + wx*newZ  =>  newPan.x = mx - cx + wW*newZ/2 - wx*newZ
  ...
  useEffect(()=>{const el=svgRef.current;const ctr=gameContainerRef.current;if(!el)return;
    ...
    return()=>{el.removeEventListener("wheel",h);if(ctr)ctr.removeEventListener("wheel",h);
      document.removeEventListener("keydown",blockBrowserZoom);document.removeEventListener("wheel",blockCtrlWheel);};},[onWh]);
  // Panel drag handlers
