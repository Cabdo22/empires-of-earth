...
export default function HexStrategyGame(){
  ...
  const[panelDragCtr,setPanelDragCtr]=useState(0); // bump to force re-render during panel drag
  ...
  const onPanelMove=useCallback(e=>{if(!draggingPanelRef.current)return;const ref=draggingPanelRef.current==="tech"?techPosRef:cityPosRef;
    ref.current={x:Math.max(0,e.clientX-dragOffsetRef.current.x),y:Math.max(0,e.clientY-dragOffsetRef.current.y)};setPanelDragCtr(c=>c+1);},[]);
  ...
  // === MINIMAP ===
  const MINIMAP_TERRAIN_COLORS={grassland:"#5a9030",forest:"#1e6a38",mountain:"#6a6a5a",water:"#2a5a8a"};
  ...
  const renderMinimap=useCallback(()=>{
    ...
    for(const h of hexes){const cx2=h.x*minimapScaleX,cy2=h.y*minimapScaleY;
      ...
      ctx.globalAlpha=vis?1:0.35;ctx.fillStyle=MINIMAP_TERRAIN_COLORS[h.terrainType]||"#444";ctx.fillRect(cx2-1.5,cy2-1.5,3,3);}
    // Cities
    ...
    // Viewport indicator — world coords of screen corners
    ctx.globalAlpha=1;const z=zoomRef.current,pan=panRef.current;
    const vpCx=window.innerWidth/2,vpCy=window.innerHeight/2;
    // flush does: translate(pan.x+vpCx-(wW*z)/2, pan.y+vpCy-(wH*z)/2) scale(z)
    // screen(0,0) → world = ((wW*z)/2 - pan.x - vpCx) / z
  ...
  minimapRenderRef.current=renderMinimap;
  useEffect(()=>{renderMinimap();},[renderMinimap]);
  ...
  const onMinimapClick=useCallback(e=>{if(!minimapRef.current)return;const r=minimapRef.current.getBoundingClientRect();
    ...
    // Reverse: pan.x = (wW*z)/2 - worldX*z (to center worldX in viewport)
  ...
  return(
    <div onMouseMove={onPanelMove} onMouseUp={onPanelUp} style={{width:"100vw",height:"100vh",background:"linear-gradient(145deg,#0a0e06 0%,#141e0c 40%,#0e1608 100%)",overflow:"hidden",position:"relative",userSelect:draggingPanelRef.current?"none":"none",fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,serif"}}>
      {/* Title */}
