...
const renderHex=(hex,vis,flags,handlers)=>{
  const{isHovered,isSelected,inMoveRange,inAttackRange,inNukeRange,units,city,player,unitSelected,settlerMode,canAct,flash,isFogged,isExplored,blockReason}=flags;
  ...
  return(
    <g key={hex.id} transform={`translate(${hex.x},${hex.y})`} onMouseEnter={onHover} onMouseLeave={onLeave} onClick={onClick} onContextMenu={onCtx} style={{cursor:settlerMode&&t!=="water"&&t!=="mountain"?"crosshair":blockReason?"not-allowed":"pointer"}}>
  ...
  const renderAll=useCallback(()=>hexes.map((hex,i)=>{
    ...
    return renderHex(hex,visData[i],{
      ...
      onCtx:e=>{e.preventDefault();e.stopPropagation();if(isPanRef.current||phase!=="MOVEMENT")return;
        ...
        if(selU&&blkReason){setMoveMsg(blkReason);setFlashes(prev=>({...prev,[uk]:"blocked"}));}},
    });
