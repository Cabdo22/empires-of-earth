...
// Data
import { TILE_SIZE, COLS, ROWS, HEX_POINTS, hexCenter, hexAt, getNeighbors, hexDist, getHexesInRadius, MAP_SIZES, setMapConfig, gameRng, FOG_SIGHT } from './data/constants.js';
...
export default function HexStrategyGame() {
  ...
  // Isometric depth-sorted render order (back-to-front by col+row)
  const renderOrder = useMemo(() => {
    const indices = hexes.map((_, i) => i);
    indices.sort((a, b) => (hexes[a].col + hexes[a].row) - (hexes[b].col + hexes[b].row));
    return indices;
  }, [hexes]);

  // === RENDER TILES ===
  const renderAll = useCallback(() => renderOrder.map(i => {
    const hex = hexes[i];
    const uk = hex.uk;
  ...
  }), [hexes, renderOrder, hovH, selH, visData, unitMap, cityMap, selU, reach, atkRange, sud, cpId, phase, players, settlerM, actable, nukeM, nukeR, flashes, fogVisible, fogExplored]);
    ...
    <div ref={gameContainerRef} onMouseMove={onPanelMove} onMouseUp={onPanelUp} style={{ width: "100vw", height: "100vh", background: "linear-gradient(145deg,#0a0e06 0%,#141e0c 40%,#0e1608 100%)", overflow: "hidden", position: "relative", userSelect: "none", touchAction: "none", fontFamily: "'Palatino Linotype','Book Antiqua',Palatino,serif" }}>
      ...
      <svg ref={svgRef} onMouseDown={onMD} onMouseMove={onMM} onMouseUp={onMU} onMouseLeave={onMU} onWheel={onWh} onContextMenu={e => e.preventDefault()} style={{ cursor: "grab", position: "absolute", top: 0, left: 0, width: "100%", height: "100%", zIndex: 1 }}>
        <defs>
          ...
          `}</style>
          <radialGradient id="gradGrass" cx="40%" cy="35%" r="70%"><stop offset="0%" stopColor="#72c040"/><stop offset="30%" stopColor="#64b038"/><stop offset="60%" stopColor="#58a030"/><stop offset="100%" stopColor="#408820"/></radialGradient>
          <radialGradient id="varGrass" cx="60%" cy="55%" r="55%"><stop offset="0%" stopColor="#4a7a1a"/><stop offset="100%" stopColor="#5a9a28" stopOpacity="0"/></radialGradient>
          <radialGradient id="gradForest" cx="40%" cy="35%" r="70%"><stop offset="0%" stopColor="#3a8830"/><stop offset="30%" stopColor="#307828"/><stop offset="60%" stopColor="#286820"/><stop offset="100%" stopColor="#1e5818"/></radialGradient>
          <radialGradient id="gradMountain" cx="40%" cy="30%" r="70%"><stop offset="0%" stopColor="#8a8878"/><stop offset="30%" stopColor="#7a7868"/><stop offset="60%" stopColor="#6a6858"/><stop offset="100%" stopColor="#5a584a"/></radialGradient>
          <radialGradient id="gradWater" cx="40%" cy="35%" r="70%"><stop offset="0%" stopColor="#4498cc"/><stop offset="30%" stopColor="#388abc"/><stop offset="60%" stopColor="#2c78a8"/><stop offset="100%" stopColor="#1c5888"/></radialGradient>
        </defs>
