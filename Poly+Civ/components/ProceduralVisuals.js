...
import { HEX_SIZE, ISO_HW, ISO_HH, hexAt, getNeighbors, NEIGHBOR_DELTAS } from '../data/constants.js';
...
// Diamond edge definitions for coastline rendering (4 edges of isometric diamond)
const SQUARE_EDGES = [
  { dx: 0, dy: -1, x0: -ISO_HW, y0: 0,       x1: 0,      y1: -ISO_HH }, // top-left edge (neighbor above)
  { dx: 1, dy: 0,  x0: 0,       y0: -ISO_HH,  x1: ISO_HW, y1: 0 },       // top-right edge (neighbor right)
  { dx: 0, dy: 1,  x0: ISO_HW,  y0: 0,        x1: 0,      y1: ISO_HH },  // bottom-right edge (neighbor below)
  { dx: -1, dy: 0, x0: 0,       y0: ISO_HH,   x1: -ISO_HW, y1: 0 },      // bottom-left edge (neighbor left)
];

// Coastline: for each tile, check which cardinal neighbors border water
export const genCoast = (hex, allHexes) => {
  if (hex.terrainType === "water") return "";
  let coast = "";
  for (const edge of SQUARE_EDGES) {
    const nc = hex.col + edge.dx, nr = hex.row + edge.dy;
    const nh = hexAt(allHexes, nc, nr);
    if (nh && nh.terrainType === "water") {
      const inset = .78;
      const mx = (edge.x0 + edge.x1) / 2 * .7, my = (edge.y0 + edge.y1) / 2 * .7;
      coast += `M${(edge.x0 * inset).toFixed(1)},${(edge.y0 * inset).toFixed(1)}Q${mx.toFixed(1)},${my.toFixed(1)},${(edge.x1 * inset).toFixed(1)},${(edge.y1 * inset).toFixed(1)}`;
    }
  }
  return coast;
};

// Water-side coastline: for water tiles bordering land
export const genWaterCoast = (hex, allHexes) => {
  ...
  for (const edge of SQUARE_EDGES) {
    ...
    if (nh && nh.terrainType !== "water") {
      const inset = .88;
      const mx = (edge.x0 + edge.x1) * .42, my = (edge.y0 + edge.y1) * .42;
      coast += `M${(edge.x0 * inset).toFixed(1)},${(edge.y0 * inset).toFixed(1)}Q${mx.toFixed(1)},${my.toFixed(1)},${(edge.x1 * inset).toFixed(1)},${(edge.y1 * inset).toFixed(1)}`;
    }
  }
  return coast;
