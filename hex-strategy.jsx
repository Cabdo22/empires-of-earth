...
export default function HexStrategyGame(){
  ...
  useEffect(()=>{const el=svgRef.current;if(!el)return;
    ...
    // Without this, pinch-zooming while cursor is over UI elements (buttons, panels) triggers
    // BROWSER zoom instead of game zoom, because those elements don't have the SVG's wheel handler.
    // Uses capture phase so it fires before any element handler, and does NOT stop propagation
    // so the SVG handler still receives the event for game zoom.
  ...
  // Counter browser pinch-zoom on the UI overlay.
  // Edge processes trackpad pinch at compositor level (bypasses preventDefault).
  // We resize the overlay to fill the visual viewport AND counter-scale the content
  // so UI elements stay their original size. rAF polling ensures near-instant response.
  useEffect(()=>{
    ...
    const sync=()=>{const el=uiOverlayRef.current;if(!el)return;
      const s=vv.scale;
      if(s>1.01||s<0.99){
        // Overlay CSS size = visual viewport * scale (in zoomed CSS pixels),
        // then transform:scale(1/s) shrinks it back to visual viewport size visually.
        // Result: overlay covers exactly the visible area, content at original size.
        el.style.width=(vv.width*s)+"px";el.style.height=(vv.height*s)+"px";
        el.style.left=vv.offsetLeft+"px";el.style.top=vv.offsetTop+"px";
        el.style.transform="scale("+(1/s)+")";el.style.transformOrigin="top left";
      }else{
    ...
    const startPoll=()=>{if(!polling){polling=true;pollLoop();}
      clearTimeout(pollTimer);pollTimer=setTimeout(()=>{polling=false;sync();},600);};
