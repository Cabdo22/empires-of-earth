...
const calcCityYields = (city, player, hexes) => {
  ...
  // Population contributes to yields: each pop adds +1 prod, +1 gold (split)
  const pop = city.population || 1;
  prod += Math.floor(pop / 2);  // +1 prod per 2 pop
  gold += Math.floor((pop + 1) / 2);  // +1 gold per 2 pop (offset by 1 so pop 1 gives +1)
...
const MILITARY_REQ_UNITS=new Set(["tank","modern_infantry","artillery","mech","fighter","bomber","panzer","marine"]);
...
const getAvailableUnits = (player, city) => {
  ...
  return Object.entries(UNIT_DEFS)
    .filter(([id, u]) => {
      ...
      // High-tier military units require Military district
...
// Unit upgrade paths: base unit → upgraded unit, with gold cost
const UPGRADE_PATHS={
...
const getUpgradeCost=(fromType)=>{const from=UNIT_DEFS[fromType],to=UNIT_DEFS[UPGRADE_PATHS[fromType]];if(!from||!to)return 0;return Math.max(2,(to.cost-from.cost)*2);};
const canUpgradeUnit=(unit,player)=>{
  const toType=UPGRADE_PATHS[unit.unitType];if(!toType)return null;
  const toDef=UNIT_DEFS[toType];if(!toDef)return null;
  if(toDef.techReq&&!player.researchedTechs.includes(toDef.techReq))return null;
  // Check civ restrictions on target
...
const refreshUnits=(player,g)=>{
  for(const unit of player.units){
    ...
    unit.movementCurrent=mv;unit.hasAttacked=false;unit.healed=false;
  }
...
export default function HexStrategyGame(){
  ...
  // Attach wheel listener as non-passive so preventDefault works — on both SVG and container
  useEffect(()=>{const el=svgRef.current;const ctr=gameContainerRef.current;if(!el)return;
    const h=e=>{e.preventDefault();e.stopPropagation();zoomRef.current=Math.min(3,Math.max(.3,zoomRef.current-e.deltaY*.001));sched();};
    el.addEventListener("wheel",h,{passive:false});
    if(ctr)ctr.addEventListener("wheel",h,{passive:false});
    // Also block Ctrl+wheel and Ctrl+plus/minus (browser zoom shortcuts) at document level
    const blockBrowserZoom=e=>{if((e.ctrlKey||e.metaKey)&&(e.key==="+"||e.key==="-"||e.key==="="||e.key==="0"))e.preventDefault();};
    const blockCtrlWheel=e=>{if(e.ctrlKey||e.metaKey)e.preventDefault();};
    document.addEventListener("keydown",blockBrowserZoom);
    document.addEventListener("wheel",blockCtrlWheel,{passive:false});
    return()=>{el.removeEventListener("wheel",h);if(ctr)ctr.removeEventListener("wheel",h);
      document.removeEventListener("keydown",blockBrowserZoom);document.removeEventListener("wheel",blockCtrlWheel);};},[sched]);
  // Panel drag handlers
  ...
  return(
    <div ref={gameContainerRef} onMouseMove={onPanelMove} onMouseUp={onPanelUp} style={{width:"100vw",height:"100vh",background:"linear-gradient(145deg,#0a0e06 0%,#141e0c 40%,#0e1608 100%)",overflow:"hidden",position:"relative",userSelect:"none",fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,serif"}}>
      {/* Title */}
