...
const PHASES=["MOVEMENT"];
const PHASE_LABELS={MOVEMENT:"Playing"};
...
const createInitialState=(civ1="Rome",civ2="China")=>{
  ...
  return{turnNumber:1,currentPlayerId:"p1",phase:"MOVEMENT",players,hexes,victoryStatus:null,nextUnitId:uidCtr,log:[`Game started. Turn 1 — ${c1.name}`],barbarians:[],eventMsg:null,rngSeed:42,rngCounter:0,explored};
};
...
const genCoast=(hex,allHexes)=>{if(hex.terrainType==="water")return"";const nb=getNeighbors(hex.col,hex.row);let coast="";for(let d=0;d<6;d++){const angle0=(d*60-30)*Math.PI/180,angle1=((d+1)*60-30)*Math.PI/180;const x0=HEX_SIZE*Math.cos(angle0),y0=HEX_SIZE*Math.sin(angle0);const x1=HEX_SIZE*Math.cos(angle1),y1=HEX_SIZE*Math.sin(angle1);const nIdx=nb.findIndex(([nc,nr])=>{const da=d===0?[-1,-1]:d===1?[0,-1]:d===2?[1,0]:d===3?[1,1]:d===4?[0,1]:[-1,0];const ec=hex.col%2===0?EVEN_COL_NEIGHBORS:ODD_COL_NEIGHBORS;return nc===hex.col+ec[d][0]&&nr===hex.row+ec[d][1];});const ec2=hex.col%2===0?EVEN_COL_NEIGHBORS:ODD_COL_NEIGHBORS;const nc=hex.col+ec2[d][0],nr=hex.row+ec2[d][1];const nh=hexAt(allHexes,nc,nr);if(nh&&nh.terrainType==="water"){const mx=(x0+x1)/2,my=(y0+y1)/2;const inset=.78;coast+=`M${(x0*inset).toFixed(1)},${(y0*inset).toFixed(1)}Q${(mx*.7).toFixed(1)},${(my*.7).toFixed(1)},${(x1*inset).toFixed(1)},${(y1*inset).toFixed(1)}`;}}return coast;};
// Water-side coastline: for water hexes bordering land
const genWaterCoast=(hex,allHexes)=>{if(hex.terrainType!=="water")return"";const ec2=hex.col%2===0?EVEN_COL_NEIGHBORS:ODD_COL_NEIGHBORS;let coast="";for(let d=0;d<6;d++){const nc=hex.col+ec2[d][0],nr=hex.row+ec2[d][1];const nh=hexAt(allHexes,nc,nr);if(nh&&nh.terrainType!=="water"){const angle0=(d*60-30)*Math.PI/180,angle1=((d+1)*60-30)*Math.PI/180;const x0=HEX_SIZE*Math.cos(angle0),y0=HEX_SIZE*Math.sin(angle0);const x1=HEX_SIZE*Math.cos(angle1),y1=HEX_SIZE*Math.sin(angle1);const inset=.88;coast+=`M${(x0*inset).toFixed(1)},${(y0*inset).toFixed(1)}Q${((x0+x1)*.42).toFixed(1)},${((y0+y1)*.42).toFixed(1)},${(x1*inset).toFixed(1)},${(y1*inset).toFixed(1)}`;}}return coast;};
...
export default function HexStrategyGame(){
  ...
  const gRef=useRef(null),svgRef=useRef(null),dirtyRef=useRef(false),gameContainerRef=useRef(null);
  // Panel drag refs
  ...
  const onWh=useCallback(e=>{e.preventDefault();e.stopPropagation();zoomRef.current=Math.min(3,Math.max(.3,zoomRef.current-e.deltaY*.001));sched();},[sched]);
  // Attach wheel listener as non-passive so preventDefault works
  useEffect(()=>{const el=svgRef.current;if(!el)return;const h=e=>{e.preventDefault();e.stopPropagation();zoomRef.current=Math.min(3,Math.max(.3,zoomRef.current-e.deltaY*.001));sched();};el.addEventListener("wheel",h,{passive:false});return()=>el.removeEventListener("wheel",h);},[sched]);
  // Pointer lock: click game area to lock mouse, Escape to unlock
  const [pointerLocked,setPointerLocked]=useState(false);
  useEffect(()=>{const onChange=()=>setPointerLocked(!!document.pointerLockElement);
    document.addEventListener("pointerlockchange",onChange);return()=>document.removeEventListener("pointerlockchange",onChange);},[]);
  const requestPointerLock=useCallback(()=>{if(!pointerLocked&&gameContainerRef.current&&!document.pointerLockElement)gameContainerRef.current.requestPointerLock?.();},[pointerLocked]);
  ...
  const doCombat = useCallback((attackerId, defCol, defRow) => {
    setGs(prev => {
      ...
      if(defender){
        const atkDmgShow=attDef.ability==="double_shot"?calcCombatPreview(attUnit,attDef,defender,UNIT_DEFS[defender.unitType],defHex?.terrainType,attPlayer,defUnit?defPlayer:{researchedTechs:[],civilization:"Barbarian"},!!defCity).aDmg*2:calcCombatPreview(attUnit,attDef,defender,UNIT_DEFS[defender.unitType],defHex?.terrainType,attPlayer,defUnit?defPlayer:{researchedTechs:[],civilization:"Barbarian"},!!defCity).aDmg;
  ...
  const onHexClick=useCallback(e=>{
    ...
    if(cE&&cE.player.id===cpId){setShowCity(cE.city.id);setSelU(null);return;}
  ...
  return(
    <div ref={gameContainerRef} onClick={requestPointerLock} onMouseMove={onPanelMove} onMouseUp={onPanelUp} style={{width:"100vw",height:"100vh",background:"linear-gradient(145deg,#0a0e06 0%,#141e0c 40%,#0e1608 100%)",overflow:"hidden",position:"relative",userSelect:"none",fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,serif"}}>
      {/* Title */}
      ...
      {/* End Turn button */}
      <div style={{position:"absolute",top:48,left:"50%",transform:"translateX(-50%)",zIndex:10,display:"flex",gap:6,alignItems:"center",background:"rgba(10,14,6,.85)",borderRadius:6,padding:"3px 8px",border:"1px solid rgba(100,140,50,.3)"}}>
      ...
      {turnPopups.length>0&&<div style={{position:"absolute",top:90,left:"50%",transform:"translateX(-50%)",zIndex:35,display:"flex",flexDirection:"column",gap:8,alignItems:"center"}}>
      ...
      <svg ref={svgRef} width="100%" height="100%" onMouseDown={onMD} onMouseMove={onMM} onMouseUp={onMU} onMouseLeave={onMU} onWheel={onWh} onContextMenu={e=>e.preventDefault()} style={{cursor:"grab"}}>
        ...
        <g ref={gRef} style={{willChange:"transform"}} onMouseMove={onHexHover} onMouseLeave={onHexLeave} onClick={onHexClick} onContextMenu={onHexCtx}>{renderAll()}
          ...
          {/* Block reason tooltip - rendered above all hexes */}
