...
export function CityPanel({ city, cp, hexes, cityPosRef, cityCollapsed, setCityCollapsed, setShowCity, onPanelDown, setProd, cancelProduction }) {
  ...
  return (
    <div data-panel="city" style={{ ...panelStyle, ...cStyle, width: 290, maxHeight: cityCollapsed ? 40 : 500, overflowY: cityCollapsed ? "hidden" : "auto", transition: "max-height .2s ease" }}>
      ...
      {!cityCollapsed && <>
        <div style={{ fontSize: 9, marginBottom: 4, display: "flex", gap: 8 }}><span>Pop:{city.population}</span><span style={{ color: "#7db840" }}>🌾{y.food}</span><span style={{ color: "#b89040" }}>⚙{y.production}</span><span style={{ color: "#60a0d0" }}>🔬{y.science}</span><span style={{ color: "#d0c050" }}>💰{y.gold}</span></div>
        <div style={{ fontSize: 8, color: "#6a7a50", marginBottom: 2 }}>Growth: {city.foodAccumulated}/{growthThreshold} <span style={{ color: surplus > 0 ? "#90d070" : "#c08040" }}>(+{Math.max(0, surplus)}/turn)</span> Eats: {foodConsumed}🌾</div>
        <div style={{ fontSize: 8, color: "#6a7a50", marginBottom: 4 }}>HP: {city.hp}/{city.hpMax || 20}</div>

        {/* Worked Tiles */}
        <div style={{ fontSize: 8, color: "#c8d8a0", marginBottom: 2 }}>Tiles ({(city.workedTileIds || []).length + 1} worked)</div>
        <div style={{ fontSize: 7, padding: "2px 4px", background: "rgba(80,120,40,.15)", borderRadius: 3, marginBottom: 1 }}>
          <span style={{ color: "#a0b880" }}>City Center ({TERRAIN_INFO[centerHex?.terrainType]?.label || "?"})</span>
          <span style={{ float: "right" }}>🌾{centerY.food} ⚙{centerY.production} 💰{centerY.gold}</span>
        </div>
        {workedTiles.map(({ hex: h, yields: ty }) => (
          <div key={h.id} style={{ fontSize: 7, padding: "2px 4px", background: "rgba(80,120,40,.1)", borderRadius: 3, marginBottom: 1 }}>
            <span style={{ color: "#8a9a70" }}>{TERRAIN_INFO[h.terrainType]?.label}{h.resource ? ` ${RESOURCE_INFO[h.resource]?.icon}` : ''}</span>
