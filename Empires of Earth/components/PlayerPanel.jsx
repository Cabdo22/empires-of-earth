  ...
  return (
    <div style={{ position: "absolute", top: 12, left: 14, zIndex: 10, background: "rgba(10,14,6,.8)", borderRadius: 6, padding: "6px 10px", border: `1px solid ${p.color}60`, minWidth: 120, pointerEvents: "auto" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 2, fontSize: 10, color: "#c8dca8" }}>
        <div>💰{p.gold} <span style={{ color: "#8a9a70" }}>+{i2.gold}/t</span></div>
        <div>🔬+{i2.science}/t{p.currentResearch && <span style={{ color: "#90c8e0" }}> →{TECH_TREE[p.currentResearch.techId]?.name} ({p.currentResearch.progress}/{TECH_TREE[p.currentResearch.techId]?.cost})</span>}</div>
        <div>🌾+{i2.food}/t ⚙+{i2.production}/t</div>
        <div style={{ color: "#8a9a6a" }}>🏛{p.cities.length} ⚔{p.units.length} 🗺{landOwned[p.id] || 0}/{totalLand}{barbarians.length > 0 && <span style={{ color: "#e06060" }}> 🏴‍☠️{barbarians.length}</span>}</div>
      </div>
    </div>
  );
}
