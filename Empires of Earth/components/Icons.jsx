...
export const UnitIcon = ({ unitType, x, y, fg, sz }) => {
  ...
  switch (unitType) {
    ...
    case "musketman": case "musketeer": case "field_cannon": case "machine_gun": return g2(<><line x1={-s * .08} y1={-s * .55} x2={-s * .08} y2={s * .2} stroke={fg} strokeWidth="1.8" /><rect x={-s * .18} y={s * .08} width={s * .28} height={s * .18} rx={s * .04} fill={bg} stroke={fg} strokeWidth="1" /><circle cx={-s * .08} cy={-s * .55} r={s * .05} fill={fg} /><path d={`M${-s * .08},${-s * .35}L${-s * .25},${-s * .25}`} stroke={fg} strokeWidth="1.2" />{unitType === "musketeer" && <><line x1={s * .12} y1={-s * .35} x2={s * .32} y2={-s * .1} stroke={fg} strokeWidth="1.5" /><line x1={s * .32} y1={-s * .1} x2={s * .25} y2={-s * .05} stroke={fg} strokeWidth="1" /></>}</>);
