...
const UNIT_DEFS={
  ...
  nuke:{name:"Nuke",icon:"☢",strength:99,hp:1,move:0,range:3,cost:15,techReq:"quantum_computing",domain:"special"},
};
...
const RANDOM_EVENTS=[
  ...
  {id:"plague",name:"Plague Strikes",desc:"Disease reduces city population.",effect:g=>{ const cp2=g.players.find(p=>p.id===g.currentPlayerId);const c=cp2.cities[Math.floor(Math.random()*cp2.cities.length)];if(c&&c.population>1){c.population--;c.foodAccumulated=0;} }},
  ...
  {id:"raid",name:"Barbarian Raid!",desc:"Barbarians attack your borders.",effect:(g,addLog2)=>{
    ...
    if(border.length>0){const bh=border[Math.floor(Math.random()*border.length)];
...
const createInitialState=(civ1="Rome",civ2="China")=>{
  ...
  for(let col=0;col<COLS;col++)for(let row=0;row<ROWS;row++){const{x,y}=hexCenter(col,row);const g=gridData[col][row];hexes.push({id:id++,col,row,x,y,terrainType:g.terrain,resource:g.resource,ownerPlayerId:null,cityId:null});}
  ...
  return{turnNumber:1,currentPlayerId:"p1",phase:"RESEARCH",players,hexes,victoryStatus:null,nextUnitId:uidCtr,log:[`Game started. Turn 1 — ${c1.name}`],barbarians:[],eventMsg:null,rngSeed:42,explored};
};
...
const getAvailableUnits = (player, city) => {
  ...
  return Object.entries(UNIT_DEFS)
    .filter(([id, u]) => {
      ...
      if (id === "nuke") return hasNuclearDistrict && player.gold >= 10;
      return true;
    })
    .map(([id, u]) => ({ id, ...u }));
};
...
// RENDER HEX
// ============================================================
const renderHex=(hex,vis,flags,handlers)=>{
  const{isHovered,isSelected,inMoveRange,inAttackRange,inNukeRange,units,city,player,unitSelected,settlerMode,canAct,flash,isFogged,isExplored}=flags;
  const{onHover,onLeave,onClick,onCtx}=handlers;
  ...
  return(
    <g key={hex.id} transform={`translate(${hex.x},${hex.y})`} onMouseEnter={onHover} onMouseLeave={onLeave} onClick={onClick} onContextMenu={onCtx} style={{cursor:settlerMode&&t!=="water"&&t!=="mountain"?"crosshair":"pointer"}}>
      ...
      {/* Units */}
      {units.length>0&&!city&&<g transform="translate(0,-6)" style={{pointerEvents:"none"}}>
        ...
        <text x={0} y={1} textAnchor="middle" dominantBaseline="middle" fontSize={16} style={{pointerEvents:"none"}}>{UNIT_DEFS[units[0].type]?.icon||"?"}</text>
        {unitSelected&&<circle cx={0} cy={0} r={20} fill="none" stroke="#60d0ff" strokeWidth="1.5" opacity=".5"/>}
        {units.length>1&&<text x={14} y={-14} textAnchor="middle" fill="#ffd740" fontSize={8} fontWeight="bold" style={{pointerEvents:"none"}}>+{units.length-1}</text>}
        {units[0].hp<(UNIT_DEFS[units[0].type]?.hp||10)&&<g transform="translate(0,16)"><rect x={-14} y={0} width={28} height={3} rx={1} fill="#333" opacity=".7"/><rect x={-14} y={0} width={28*(units[0].hp/(UNIT_DEFS[units[0].type]?.hp||10))} height={3} rx={1} fill={units[0].hp>(UNIT_DEFS[units[0].type]?.hp||10)*.5?"#4a4":"#c44"} opacity=".9"/></g>}
      </g>}
      {units.length>0&&city&&<g transform="translate(28,-20)" style={{pointerEvents:"none"}}>
        ...
        <text x={0} y={1} textAnchor="middle" dominantBaseline="middle" fontSize={11} style={{pointerEvents:"none"}}>{UNIT_DEFS[units[0].type]?.icon||"?"}</text>
        {units.length>1&&<text x={10} y={-8} fill="#ffd740" fontSize={7} fontWeight="bold">+{units.length-1}</text>}
      </g>}

      {!city&&units.length===0&&<text x={0} y={2} textAnchor="middle" dominantBaseline="middle" fill={t==="water"?"rgba(150,200,240,.2)":t==="mountain"?"rgba(200,200,200,.18)":"rgba(200,216,160,.18)"} fontSize={8} fontFamily="monospace" style={{pointerEvents:"none"}}>{hex.col},{hex.row}</text>}
      ...
      {flash&&<polygon points={HEX_POINTS} fill={flash==="nuke"?"rgba(255,200,0,.5)":"rgba(255,80,80,.35)"} stroke={flash==="nuke"?"#ff8000":"#ff2020"} strokeWidth="3"><animate attributeName="opacity" from="1" to="0" dur="0.8s" fill="freeze"/></polygon>}
    </g>
  );
};
...
export default function HexStrategyGame(){
  ...
  const flush=useCallback(()=>{const z=zoomRef.current,p=panRef.current,cx=window.innerWidth/2,cy=window.innerHeight/2;if(gRef.current)gRef.current.style.transform=`translate(${p.x+cx-(wW*z)/2}px,${p.y+cy-(wH*z)/2}px) scale(${z})`;dirtyRef.current=false;},[wW,wH]);
  ...
  const launchNuke=useCallback((nuId,tc,tr)=>{
    setGs(prev=>{const g=JSON.parse(JSON.stringify(prev));const aP=g.players.find(p=>p.id===g.currentPlayerId);const dP=g.players.find(p=>p.id!==g.currentPlayerId);
      ...
      for(const bh of blast){const k=`${bh.col},${bh.row}`;fl[k]="nuke";
        ...
        if(dc){dc.hp=(dc.hp||20)-15;if(dc.hp<=0){dP.cities=dP.cities.filter(c=>c.id!==dc.id);dc.hp=5;dc.hpMax=20;aP.cities.push(dc);bh.ownerPlayerId=aP.id;addLog(`☢ ${dc.name} obliterated!`,g);}
          else addLog(`☢ ${dc.name} hit! (${dc.hp}HP)`,g);}}
    ...
    if (city.currentProduction) {
      ...
      if (def && city.productionProgress >= def.cost) {
        if (isUnit) {
          ...
          if (city.currentProduction.itemId === "nuke") player.gold -= 10;
        } else {
  ...
  const renderAll=useCallback(()=>hexes.map((hex,i)=>{
    ...
    return renderHex(hex,visData[i],{
      isHovered:hovH===hex.id,isSelected:selH===hex.id,inMoveRange:inMv,inAttackRange:inMelee||inRng,inNukeRange:inNk,
      unitSelected:uSel,units:fogged?[]:uH.map(u=>({...u,type:u.unitType,hp:u.hpCurrent})),
      city:fogged?null:(cE?.city||null),player:cE?.player||ownerP,settlerMode:!!settlerM,canAct:canA,flash:flashes[uk]||null,isFogged:fogged,isExplored:isExplored2,
    },{
      onHover:()=>{if(isPanRef.current||fogged)return;setHovH(hex.id);
        if((inMelee||inRng)&&sud&&eU.length>0){const eu=eU[0];
      ...
      onLeave:()=>{setHovH(null);setPreview(null);},
      onClick:e=>{e.stopPropagation();if(isPanRef.current||fogged)return;
        ...
        if(isMy&&!uSel&&phase==="MOVEMENT"){setSelU(myU[0].id);setSelH(null);}
        else if(isMy&&uSel){
      ...
      onCtx:e=>{e.preventDefault();e.stopPropagation();if(isPanRef.current||phase!=="MOVEMENT")return;
        if(selU&&(inMelee||inRng)){doCombat(selU,hex.col,hex.row);return;}
        if(selU&&inMv){moveU(selU,hex.col,hex.row);return;}},
    });
  }),[hexes,hovH,selH,visData,unitMap,cityMap,selU,reach,atkRange,sud,cpId,phase,players,cp,moveU,settlerM,foundCity,doCombat,actable,nukeM,nukeR,launchNuke,flashes,fogVisible,fogExplored]);
  ...
  if(!gameStarted||!gs){
    ...
    return(<div style={{width:"100vw",height:"100vh",background:"radial-gradient(ellipse at center,#1a2a10 0%,#0a0e06 70%)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",fontFamily:"'Palatino Linotype',serif",gap:24}}>
      ...
      <div style={{color:"#6a7a50",fontSize:12,letterSpacing:3}}>Choose Your Civilizations</div>
      <div style={{display:"flex",gap:40}}>
        {["p1","p2"].map(pid=>{const label=pid==="p1"?"Player 1":"Player 2";const picked=civPick[pid];
          return(<div key={pid} style={{textAlign:"center"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {civKeys.map(ck=>{const cv=CIV_DEFS[ck];const sel=picked===ck;const taken=civPick[pid==="p1"?"p2":"p1"]===ck;
                return(<div key={ck} onClick={()=>{if(!taken){SFX.click();setCivPick(prev=>({...prev,[pid]:ck}));}}}
                  style={{padding:"10px 20px",borderRadius:6,cursor:taken?"not-allowed":"pointer",
                    ...
                    border:`1px solid ${sel?cv.color:taken?"#333":"#3a4a2a"}`,opacity:taken?.4:1,minWidth:200,textAlign:"left"}}>
      ...
      <button onClick={()=>{SFX.found();setGs(createInitialState(civPick.p1,civPick.p2));setGameStarted(true);}}
  ...
  if(gs.victoryStatus){const w=players.find(p=>p.id===gs.victoryStatus.winner);if(!victoryPlayed.current){victoryPlayed.current=true;SFX.victory();}
    return(<div style={{width:"100vw",height:"100vh",background:"radial-gradient(ellipse at center,#1a2a10 0%,#0a0e06 70%)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",fontFamily:"'Palatino Linotype',serif"}}>
      ...
      <button onClick={()=>{uidCtr=0;setGs(null);setGameStarted(false);setSelU(null);setSelH(null);victoryPlayed.current=false;}} style={{...btnStyle(true),marginTop:24,fontSize:14,padding:"8px 24px"}}>New Game</button>
    </div>);}

  return(
    <div style={{width:"100vw",height:"100vh",background:"linear-gradient(145deg,#0a0e06 0%,#141e0c 40%,#0e1608 100%)",overflow:"hidden",position:"relative",userSelect:"none",fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,serif"}}>
      {/* Title */}
      ...
      {/* Tech tree */}
      {showTech&&<div style={{...panelStyle,top:95,left:"50%",transform:"translateX(-50%)",width:Math.min(720,window.innerWidth-40),maxHeight:320,overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,color:"#c8d8a0",letterSpacing:2}}>TECHNOLOGY TREE</span><button onClick={()=>setShowTech(false)} style={{...btnStyle(false),fontSize:10}}>✕</button></div>
      ...
      {showCity&&(()=>{const city=cp.cities.find(c=>c.id===showCity);if(!city)return null;const y=calcCityYields(city,cp,hexes);const avU=getAvailableUnits(cp,city);const avD=getAvailableDistricts(cp,city);
        return(<div style={{...panelStyle,top:95,right:14,width:280,maxHeight:420,overflowY:"auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:13,color:"#ffd740",letterSpacing:2}}>{city.name}</span><button onClick={()=>setShowCity(null)} style={{...btnStyle(false),fontSize:10}}>✕</button></div>
      ...
      <svg ref={svgRef} width="100%" height="100%" onMouseDown={onMD} onMouseMove={onMM} onMouseUp={onMU} onMouseLeave={onMU} onWheel={onWh} onContextMenu={e=>e.preventDefault()} style={{cursor:"grab"}}>
        ...
        <g ref={gRef} style={{willChange:"transform"}}>{renderAll()}</g>
      </svg>
    </div>
  );
}
