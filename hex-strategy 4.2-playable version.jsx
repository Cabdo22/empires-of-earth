...
let COLS = 10, ROWS = 10;
let P1_START = { col: 2, row: 5 }, P2_START = { col: 8, row: 3 };
...
const setMapConfig = (sizeKey) => {
  const cfg = MAP_SIZES[sizeKey] || MAP_SIZES.small;
  COLS = cfg.cols; ROWS = cfg.rows;
  // Place starts in opposite quadrants, away from edges
...
const hexAt = (hexes, col, row) => hexes[col * ROWS + row];
...
const UNIT_DEFS={
  ...
  chu_ko_nu:{name:"Chu-Ko-Nu",icon:"🏹",strength:3,hp:12,move:2,range:2,cost:5,techReq:"hunting",domain:"land",civReq:"China",replaces:"archer",ability:"double_shot"},
...
const calcCombatPreview = (attUnit, attDef, defUnit, defDef, defTerrain, attPlayer, defPlayer, inCity) => {
  ...
  // Ottoman siege bonus: +1 str for siege units (catapult, great_bombard, battleship)
...
const generateMap = () => {
  ...
  // Scale terrain targets proportionally to map area (base: 100 hexes)
...
const createInitialState=(civ1="Rome",civ2="China")=>{
  ...
  const players=[
    {id:"p1",civilization:civ1,name:c1.name,color:c1.color,colorBg:c1.colorBg,colorLight:c1.colorLight,gold:5,researchedTechs:["basic_tools"],currentResearch:null,
    ...
    {id:"p2",civilization:civ2,name:c2.name,color:c2.color,colorBg:c2.colorBg,colorLight:c2.colorLight,gold:5,researchedTechs:["basic_tools"],currentResearch:null,
...
const calcCityYields = (city, player, hexes) => {
  ...
  if (player.civilization === "England") {
    for (const [nc, nr] of getNeighbors(cityHex.col, cityHex.row)) {
      ...
      if (nh?.terrainType === "water") { gold += 1; break; } // +1 if any adjacent water
    }
  }
...
const getAvailableUnits = (player, city) => {
  ...
  // Collect which base units are replaced by this civ's unique units
  ...
  return Object.entries(UNIT_DEFS)
    .filter(([id, u]) => {
      ...
      // Hide base unit if this civ has a unique replacement
      if (replacedByUnique.has(id)) return false;
      // Hide other civs' unique units
...
const getMoveCost = (terrain, domain, ability) => {
  if (domain === "air") return 1;
  if (domain === "sea") return terrain === "water" ? 1 : null;
  if (domain === "amphibious") {
    if (terrain === "water") return ability === "water_speed" ? 1 : 2; // Marine: water costs 1
    const cost = TERRAIN_INFO[terrain].moveCost;
    return cost === null ? null : cost;
  }
  // Land unit
  const info = TERRAIN_INFO[terrain];
  if (ability === "forest_move" && terrain === "forest") return 1; // Musketeer: no forest penalty
...
const getReachableHexes = (startCol, startRow, movePoints, hexes, domain = "land", playerId = null, allPlayers = null, ability = null) => {
...
// ============================================================
// SHARED TURN-PROCESSING HELPERS (used by both player and AI)
// ============================================================
const addLogMsg=(msg,g)=>{g.log=[...(g.log||[]).slice(-30),msg];};
...
const processCityTurn=(city,player,g,sfxQ)=>{
  ...
  if(city.currentProduction){
    ...
    // Germany: -1 production cost for units (min 1)
    ...
    if(def&&city.productionProgress>=effCost){
      if(isUnit){
        ...
        g.nextUnitId=(g.nextUnitId||0)+1;
        player.units.push({id:`${player.id}-u${g.nextUnitId}`,unitType:city.currentProduction.itemId,
          hexCol:cityHex.col,hexRow:cityHex.row,movementCurrent:def.move,hpCurrent:def.hp,hasAttacked:false});
        if(city.currentProduction.itemId==="nuke")player.gold-=15;
      }else{city.districts.push(city.currentProduction.itemId);}
      addLogMsg(`${city.name} built ${def.name}!`,g);
      if(sfxQ)sfxQ.push("build");
      city.currentProduction=null;city.productionProgress=0;
    }
  }
  city.foodAccumulated+=yields.food;
  const growthThreshold=city.population*10;
  if(city.foodAccumulated>=growthThreshold){city.population++;city.foodAccumulated-=growthThreshold;addLogMsg(`${city.name} grew to pop ${city.population}!`,g);}
  if(city.hp<(city.hpMax||20))city.hp=Math.min(city.hpMax||20,city.hp+2);
};

const expandTerritory=(player,g)=>{
  for(const city of player.cities){const cityHex=g.hexes[city.hexId];
    for(const[nc,nr]of getNeighbors(cityHex.col,cityHex.row)){
...
const healGarrison=(player,hexes)=>{
  for(const unit of player.units){
    const inCity=player.cities.some(c=>{const h=hexes[c.hexId];return h.col===unit.hexCol&&h.row===unit.hexRow;});
    if(inCity){const maxHp=UNIT_DEFS[unit.unitType]?.hp||10;unit.hpCurrent=Math.min(maxHp,unit.hpCurrent+3);}
  }
};
...
const refreshUnits=(player,g)=>{
  for(const unit of player.units){
    ...
    let mv=def?.move||2;
    // England civ bonus: +1 movement for naval units
    if(player.civilization==="England"&&def?.domain==="sea")mv+=1;
    unit.movementCurrent=mv;unit.hasAttacked=false;
  }
...
const spawnBarbarians=(g)=>{
  ...
  g.nextUnitId=(g.nextUnitId||0)+1;
  g.barbarians.push({id:`barb-${g.nextUnitId}`,unitType:barbType,hexCol:spawnHex.col,hexRow:spawnHex.row,hpCurrent:barbDef.hp,movementCurrent:barbDef.move,hasAttacked:false});
...
const findNearestTarget=(barb,players,hexes)=>{
  ...
  for(const player of players){
    for(const unit of player.units){const dist=hexDist(barb.hexCol,barb.hexRow,unit.hexCol,unit.hexRow);if(dist<bestDist){bestDist=dist;bestHex={col:unit.hexCol,row:unit.hexRow};}}
    for(const city of player.cities){const cityH=hexes[city.hexId];const dist=hexDist(barb.hexCol,barb.hexRow,cityH.col,cityH.row);if(dist<bestDist){bestDist=dist;bestHex={col:cityH.col,row:cityH.row};}}
  }
...
const processBarbarians=(g)=>{
  for(const barb of g.barbarians){
    barb.movementCurrent=UNIT_DEFS[barb.unitType]?.move||2;barb.hasAttacked=false;
    const{bestHex,bestDist}=findNearestTarget(barb,g.players,g.hexes);
    if(bestHex&&bestDist>1){
      const neighbors=getNeighbors(barb.hexCol,barb.hexRow);let moveTarget=null,moveTargetDist=Infinity;
      for(const[nc,nr]of neighbors){const hex=hexAt(g.hexes,nc,nr);if(!hex||hex.terrainType==="water"||hex.terrainType==="mountain")continue;
        const dist=hexDist(nc,nr,bestHex.col,bestHex.row);if(dist<moveTargetDist){moveTargetDist=dist;moveTarget={col:nc,row:nr};}}
      if(moveTarget){barb.hexCol=moveTarget.col;barb.hexRow=moveTarget.row;}
    }
    if(bestDist<=1&&!barb.hasAttacked){
      for(const player of g.players){
        const target=player.units.find(u=>u.hexCol===bestHex.col&&u.hexRow===bestHex.row);if(!target)continue;
        ...
        if(target.hpCurrent<=0){player.units=player.units.filter(u=>u.id!==target.id);addLogMsg(`☠ ${targetDef.name} killed by barbarians!`,g);}
        if(barb.hpCurrent<=0){g.barbarians=g.barbarians.filter(b=>b.id!==barb.id);addLogMsg(`☠ Barbarian ${barbDef.name} destroyed!`,g);}
        break;
      }
    }
  }
...
const rollRandomEvent=(g)=>{
  if(gameRng(g)<0.20){const evt=RANDOM_EVENTS[Math.floor(gameRng(g)*RANDOM_EVENTS.length)];
    evt.effect(g,addLogMsg);g.eventMsg={name:evt.name,desc:evt.desc};addLogMsg(`🎲 Event: ${evt.name} — ${evt.desc}`,g);
  }else{g.eventMsg=null;}
};

const checkVictoryState=(g)=>{
  ...
  for(const p of g.players){if(p.researchedTechs.includes("quantum_computing")&&p.researchedTechs.includes("fusion_power")){g.victoryStatus={winner:p.id,type:"Science"};return;}}
...
const UnitIcon=({unitType,x,y,fg,sz})=>{
  ...
  switch(unitType){
    case"scout":return g2(<><circle cx={0} cy={-s*.1} r={s*.3} fill="none" stroke={fg} strokeWidth="1.5"/><circle cx={0} cy={-s*.1} r={s*.12} fill={fg}/><line x1={0} y1={s*.2} x2={0} y2={s*.5} stroke={fg} strokeWidth="1.2"/></>);
    case"warrior":return g2(<><circle cx={0} cy={-s*.25} r={s*.25} fill="none" stroke={fg} strokeWidth="1.2"/><line x1={-s*.35} y1={-s*.1} x2={s*.35} y2={-s*.1} stroke={fg} strokeWidth="1.5"/><rect x={-s*.12} y={-s*.1} width={s*.24} height={s*.55} rx={s*.06} fill="none" stroke={fg} strokeWidth="1.2"/></>);
    case"settler":return g2(<><rect x={-s*.35} y={-s*.3} width={s*.7} height={s*.5} rx={s*.08} fill="none" stroke={fg} strokeWidth="1.2"/><line x1={0} y1={-s*.3} x2={0} y2={-s*.5} stroke={fg} strokeWidth="1"/><polygon points={`${s*.02},${-s*.5} ${s*.25},${-s*.4} ${s*.02},${-s*.3}`} fill={fg}/></>);
    case"archer":case"chu_ko_nu":return g2(<><path d={`M${-s*.15},${-s*.45}Q${-s*.4},0,${-s*.15},${s*.45}`} fill="none" stroke={fg} strokeWidth="1.3"/><line x1={-s*.15} y1={-s*.35} x2={-s*.15} y2={s*.35} stroke={fg} strokeWidth="1"/><line x1={-s*.1} y1={0} x2={s*.45} y2={0} stroke={fg} strokeWidth="1.2"/><polygon points={`${s*.45},0 ${s*.3},${-s*.1} ${s*.3},${s*.1}`} fill={fg}/>{unitType==="chu_ko_nu"&&<line x1={-s*.1} y1={s*.15} x2={s*.35} y2={s*.15} stroke={fg} strokeWidth=".8" opacity=".7"/>}</>);
    case"swordsman":case"legionary":return g2(<><line x1={0} y1={-s*.5} x2={0} y2={s*.15} stroke={fg} strokeWidth="1.8"/><line x1={-s*.2} y1={-s*.35} x2={s*.2} y2={-s*.35} stroke={fg} strokeWidth="1.5"/><ellipse cx={0} cy={s*.3} rx={s*.3} ry={s*.2} fill="none" stroke={fg} strokeWidth="1.2"/>{unitType==="legionary"&&<line x1={-s*.3} y1={s*.3} x2={s*.3} y2={s*.3} stroke={fg} strokeWidth=".8"/>}</>);
    case"knight":case"war_chariot":return g2(<><ellipse cx={0} cy={s*.05} rx={s*.3} ry={s*.25} fill="none" stroke={fg} strokeWidth="1.2"/><path d={`M${-s*.15},${-s*.2}Q0,${-s*.55},${s*.15},${-s*.2}`} fill="none" stroke={fg} strokeWidth="1.2"/><circle cx={s*.15} cy={-s*.15} r={s*.06} fill={fg}/>{unitType==="war_chariot"&&<line x1={-s*.35} y1={s*.3} x2={s*.35} y2={s*.3} stroke={fg} strokeWidth="1.2"/>}</>);
    case"catapult":case"great_bombard":return g2(<><line x1={-s*.3} y1={s*.3} x2={s*.3} y2={s*.3} stroke={fg} strokeWidth="1.5"/><line x1={-s*.2} y1={s*.3} x2={s*.1} y2={-s*.3} stroke={fg} strokeWidth="1.3"/><circle cx={s*.2} cy={-s*.2} r={s*.15} fill={unitType==="great_bombard"?fg:"none"} stroke={fg} strokeWidth="1"/></>);
    case"tank":case"panzer":return g2(<><rect x={-s*.4} y={-s*.15} width={s*.8} height={s*.4} rx={s*.08} fill="none" stroke={fg} strokeWidth="1.3"/><rect x={-s*.1} y={-s*.35} width={s*.55} height={s*.2} rx={s*.05} fill="none" stroke={fg} strokeWidth="1"/><circle cx={s*.35} cy={-s*.25} r={s*.06} fill={fg}/>{unitType==="panzer"&&<line x1={-s*.4} y1={s*.1} x2={s*.4} y2={s*.1} stroke={fg} strokeWidth=".8"/>}</>);
    case"musketman":case"musketeer":return g2(<><line x1={-s*.05} y1={-s*.5} x2={-s*.05} y2={s*.25} stroke={fg} strokeWidth="1.5"/><rect x={-s*.15} y={s*.1} width={s*.25} height={s*.15} rx={s*.03} fill="none" stroke={fg} strokeWidth="1"/><circle cx={-s*.05} cy={-s*.5} r={s*.06} fill={fg}/>{unitType==="musketeer"&&<line x1={s*.1} y1={-s*.3} x2={s*.3} y2={-s*.1} stroke={fg} strokeWidth="1.2"/>}</>);
    case"modern_infantry":case"marine":return g2(<><circle cx={0} cy={-s*.3} r={s*.2} fill="none" stroke={fg} strokeWidth="1.2"/><line x1={0} y1={-s*.1} x2={0} y2={s*.3} stroke={fg} strokeWidth="1.3"/><line x1={-s*.3} y1={s*.05} x2={s*.3} y2={s*.05} stroke={fg} strokeWidth="1.2"/><line x1={-s*.15} y1={s*.3} x2={-s*.25} y2={s*.5} stroke={fg} strokeWidth="1.2"/><line x1={s*.15} y1={s*.3} x2={s*.25} y2={s*.5} stroke={fg} strokeWidth="1.2"/>{unitType==="marine"&&<path d={`M${-s*.2},${-s*.45}L${s*.2},${-s*.45}`} stroke={fg} strokeWidth="1"/>}</>);
    case"galley":return g2(<><path d={`M${-s*.4},${s*.1}Q0,${s*.4},${s*.4},${s*.1}`} fill="none" stroke={fg} strokeWidth="1.3"/><line x1={0} y1={s*.1} x2={0} y2={-s*.35} stroke={fg} strokeWidth="1.2"/><polygon points={`0,${-s*.35} ${s*.25},${-s*.1} 0,${-s*.1}`} fill={fg} opacity=".7"/></>);
    case"destroyer":case"man_o_war":return g2(<><path d={`M${-s*.4},${s*.05}L${-s*.3},${s*.2}L${s*.3},${s*.2}L${s*.4},${s*.05}Z`} fill="none" stroke={fg} strokeWidth="1.2"/><line x1={0} y1={s*.05} x2={0} y2={-s*.4} stroke={fg} strokeWidth="1.3"/>{unitType==="man_o_war"?<><polygon points={`0,${-s*.4} ${s*.3},${-s*.2} 0,${-s*.15}`} fill={fg} opacity=".6"/><polygon points={`0,${-s*.15} ${s*.25},${s*.0} 0,${s*.05}`} fill={fg} opacity=".4"/></>:<polygon points={`0,${-s*.4} ${s*.3},${-s*.15} 0,${s*.05}`} fill={fg} opacity=".6"/>}</>);
    case"battleship":return g2(<><rect x={-s*.4} y={-s*.1} width={s*.8} height={s*.25} rx={s*.06} fill="none" stroke={fg} strokeWidth="1.3"/><line x1={-s*.1} y1={-s*.1} x2={-s*.1} y2={-s*.35} stroke={fg} strokeWidth="1.5"/><line x1={s*.15} y1={-s*.1} x2={s*.15} y2={-s*.3} stroke={fg} strokeWidth="1.2"/><circle cx={-s*.1} cy={-s*.35} r={s*.06} fill={fg}/></>);
    case"fighter":return g2(<><line x1={0} y1={-s*.5} x2={0} y2={s*.4} stroke={fg} strokeWidth="1.5"/><line x1={-s*.45} y1={s*.05} x2={s*.45} y2={s*.05} stroke={fg} strokeWidth="1.3"/><line x1={-s*.2} y1={s*.35} x2={s*.2} y2={s*.35} stroke={fg} strokeWidth="1"/><polygon points={`0,${-s*.5} ${-s*.08},${-s*.35} ${s*.08},${-s*.35}`} fill={fg}/></>);
    case"bomber":return g2(<><ellipse cx={0} cy={0} rx={s*.15} ry={s*.4} fill="none" stroke={fg} strokeWidth="1.3"/><line x1={-s*.45} y1={0} x2={s*.45} y2={0} stroke={fg} strokeWidth="1.5"/><line x1={-s*.2} y1={s*.35} x2={s*.2} y2={s*.35} stroke={fg} strokeWidth="1"/><circle cx={0} cy={s*.25} r={s*.08} fill={fg}/></>);
    case"nuke":return g2(<><circle cx={0} cy={-s*.1} r={s*.3} fill="none" stroke={fg} strokeWidth="1.2"/><path d={`M${-s*.15},${-s*.15}L0,${-s*.35}L${s*.15},${-s*.15}`} fill="none" stroke={fg} strokeWidth="1"/><path d={`M${-s*.15},${s*.05}L0,${s*.25}L${s*.15},${s*.05}`} fill="none" stroke={fg} strokeWidth="1"/><circle cx={0} cy={-s*.05} r={s*.08} fill={fg}/></>);
    case"jaguar":return g2(<><circle cx={0} cy={-s*.25} r={s*.22} fill="none" stroke={fg} strokeWidth="1.2"/><circle cx={-s*.08} cy={-s*.28} r={s*.04} fill={fg}/><circle cx={s*.08} cy={-s*.28} r={s*.04} fill={fg}/><line x1={0} y1={-s*.03} x2={0} y2={s*.25} stroke={fg} strokeWidth="1.3"/><line x1={-s*.25} y1={s*.05} x2={s*.25} y2={s*.05} stroke={fg} strokeWidth="1.2"/><path d={`M${s*.2},${s*.25}Q${s*.35},${s*.15},${s*.4},${s*.3}`} fill="none" stroke={fg} strokeWidth="1"/></>);
    ...
    if (!unit.hasAttacked) {
      ...
      for (const eu of enemyPlayer.units) {
        ...
        const preview = calcCombatPreview(
          ...
          hexAt(g.hexes, eu.hexCol, eu.hexRow)?.terrainType,
...
const aiExecuteTurn = (gameState) => {
  ...
  // City phase: set idle productions, then run shared processing
...
export default function HexStrategyGame(){
  ...
  const doCombat = useCallback((attackerId, defCol, defRow) => {
    setGs(prev => {
      ...
      if (defender) {
        ...
        // Chu-Ko-Nu double shot: apply attack damage twice
        const atkDmg = attDef.ability === "double_shot" ? preview.aDmg * 2 : preview.aDmg;
        ...
        let msg = `${attDef.name}→${barbUnit ? "Barb " : ""}${defDef.name}: ${atkDmg}dmg${attDef.ability === "double_shot" ? " (x2)" : ""}`;
        if (preview.dDmg > 0) msg += ` took ${preview.dDmg}`;

        // Defender killed (use actual HP after damage, not preview which doesn't account for double_shot)
  ...
  const onHexHover=useCallback(e=>{
    ...
    if(selU&&phase==="MOVEMENT"&&sud){
      ...
      if((inMelee2||inRng2)&&eU.length>0){const eu=eU[0];
        ...
        const effADmg=sud.def.ability==="double_shot"?pv.aDmg*2:pv.aDmg;
        setPreview({...pv,aDmg:effADmg,an:sud.def.name,dn:UNIT_DEFS[eu.unitType]?.name,ahp:sud.hpCurrent,dhp:eu.hpCurrent,dblShot:sud.def.ability==="double_shot"});
  ...
  // === CIV SELECTION SCREEN (sequential: P1 picks, then P2 picks) ===
  if(!gameStarted||!gs){
  ...
  if(gs.victoryStatus){const w=players.find(p=>p.id===gs.victoryStatus.winner);if(!victoryPlayed.current){victoryPlayed.current=true;SFX.victory();}
    return(<div style={{width:"100vw",height:"100vh",background:"radial-gradient(ellipse at center,#1a2a10 0%,#0a0e06 70%)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",fontFamily:"'Palatino Linotype',serif"}}>
      ...
      <button onClick={()=>{uidCtr=0;setGs(null);setGameStarted(false);setGameMode(null);setMapSizePick(null);setSelU(null);setSelH(null);setAiThinking(false);setTutorialOn(true);setTutorialDismissed({});victoryPlayed.current=false;techPosRef.current={x:null,y:95};cityPosRef.current={x:null,y:95};setTechCollapsed(false);setCityCollapsed(false);setCivPickStep(1);setTurnTransition(null);prevCpId.current=null;}} style={{...btnStyle(true),marginTop:24,fontSize:14,padding:"8px 24px"}}>New Game</button>
    </div>);}
  ...
  return(
    <div onMouseMove={onPanelMove} onMouseUp={onPanelUp} style={{width:"100vw",height:"100vh",background:"linear-gradient(145deg,#0a0e06 0%,#141e0c 40%,#0e1608 100%)",overflow:"hidden",position:"relative",userSelect:"none",fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,serif"}}>
      ...
      {(()=>{const p=cp;const i2=calcPlayerIncome(p,hexes);return(
        <div style={{position:"absolute",top:12,left:14,zIndex:10,background:"rgba(10,14,6,.8)",borderRadius:6,padding:"6px 10px",border:`1px solid ${p.color}60`,minWidth:120}}>
