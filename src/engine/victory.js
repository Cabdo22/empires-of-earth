// ============================================================
// VICTORY — win condition checks
// ============================================================
import { TERRITORIAL_WIN } from './constants.js';
...
export const checkVictoryState = (g) => {
  ...
  for (const p of g.players) {
    if (p.researchedTechs.includes("quantum_computing") && p.researchedTechs.includes("fusion_power")) {
      g.victoryStatus = { winner: p.id, type: "Science" }; return;
    }
  }
  ...
  for (const p of g.players) {
    ...
    if (own >= Math.ceil(land.length * TERRITORIAL_WIN)) {
      g.victoryStatus = { winner: p.id, type: "Territorial" }; return;
    }
  }
};
